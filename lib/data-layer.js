...
import { getCsrfToken, rateLimitClient, SECURITY_LIMITS } from './client-security';
...
export async function createOrder(order) { if (!rateLimitClient('checkout', SECURITY_LIMITS.checkout.limit, SECURITY_LIMITS.checkout.windowMs)) throw new Error('Too many checkout attempts. Please wait and try again.'); const x = prep('orders', order); if (isSupabaseConfigured) { const { data, error } = await supabase.functions.invoke('secure-checkout', { body: { payload: x }, headers: { 'x-bj-client': 'storefront', 'x-bj-csrf': getCsrfToken() } }); if (error) throw error; if (data?.error) throw new Error(data.error); return normOrder(data?.order || x); } return saveEntity('orders', order); }
