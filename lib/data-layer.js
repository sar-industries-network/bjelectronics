...
export async function loadState() {
  ...
  try {
    ...
    const error = [p.error, c.error, o.error, cu.error, pr.error, s.error].find(Boolean);
    if (error) throw error;
    const local = readLocal();
    const state = { ...local, products: (p.data || []).map(normProduct), categories: (c.data || []).map(normCategory), orders: (o.data || []).map(normOrder), customers: (cu.data || []).map(normCustomer), promotions: (pr.data || []).map(normPromo), settings: normSettings(s.data?.[0], local.settings) };
    writeLocal(state);
    return state;
...
export async function createOrder(order) { if (isSupabaseConfigured) { try { const { data, error } = await supabase.rpc('place_order_public', { payload: order }); if (error) throw error; return normOrder(data || order); } catch (e) { console.warn('RPC checkout failed:', e); } } return saveEntity('orders', order); }
